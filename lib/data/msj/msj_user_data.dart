  ...
  Future<Result<User>> createUser(
    ...
    try {
      final response = await _dio!.post(
        'http://192.168.18.253:3000/user/create-user',
        options: _options,
        data: {
          "userId": userId,
          "photoUrl": photoUrl,
  ...
  Future<Result<User>> getUser({required String userId}) async {
    try {
      final response = await _dio!.get(
        'http://192.168.18.253:3000/user/$userId',
        options: _options,
      );
  ...
  Future<Result<String>> verifyAccount({required String userId}) async {
    try {
      final response = await _dio!.put(
        'http://192.168.18.253:3000/user/update-user-verification/$userId',
        options: _options,
      );
